...
_阅读 Python 官方文档时所做的一些快速笔记；之后体系化学习，再另起炉灶。_

##### 数据结构

- **字符串和列表**：序列，有序。可进行索引操作、切片操作。
  - 包含诸多列表方法，如 pop()
  - 可以用以表示栈和队列
  - **列表推导式**： 如
```python
...
- **元组**：序列，用圆括号括住，immutable。
- **集合**：不重复而无序。使用 set() 创建空集合。使用**花括号**表示。
- **字典**：使用**花括号**括住的**键值对**，可以用 dict([(), (), ()]) 构造函数。
  - .keys() 取关键字
  - .items() 取关键字和值
  - .enumerate() 取索引和值
...
```python
print('the story of {0} and {1} and {other}.'.format('Bill', 'Manfred', other = 'Georg')

print(f"the story of {var1} and {var2}")
```
...
实例对象会作为函数的第一个参数传入。例如， x.f() 等价于 Classname.f(x)

#### 深浅拷贝

##### 总结对照表

| 特性      | Python 实现                                         | C++ 近似类比                             | JS 近似类比                                               | 修改第一层是否影响原变量 | 修改深层是否影响原变量 |
| ------- | ------------------------------------------------- | ------------------------------------ | ----------------------------------------------------- | ------------ | ----------- |
| **赋值**  | `b = a`                                           | `T& b = a;`                          | `b = a`                                               | **是**        | **是**       |
| **浅拷贝** | `b = a[:]`  <br>`b = list(a)`  <br>`copy.copy(a)` | `std::vector<T*> b(a);`  <br>(指针的副本) | `b = [...a]`  <br>`Object.assign`                     | 否            | **是**       |
| **深拷贝** | `copy.deepcopy(a)`                                | 自定义深拷贝构造函数   <br>(递归复制指向内容)          | `_.cloneDeep(a)`  <br>`JSON.parse(JSON.stringify(a))` | 否            | 否           |
### Python OOP
...
class MyClass:
	...
	def ShowNumber(self):
		MyClass.static_a += 1
		MyClass.__private_static_b += 1
		self.c += 1
		self.__d += 1
		print(f"A: {MyClass.static_a} \tB: {MyClass.__private_static_b} \tC: {self.c} \tD: {self.__d}")

print(f"A：所有实例共享，直接访问（相当于静态变量）: {MyClass.static_a}")
# print(MyClass.__private_static_b) # AttributeError，不可直接访问

# 实例变量独属于每个实例对象
# print(f"实例变量C: {MyClass.c}") # AttributeError: type object 'MyClass' has no attribute 'c'
x = MyClass(1, 1)
y = MyClass(99, 99)
print(f"x-c: {x.c} \t y-c: {y.c}") # x-c: 1 	 y-c: 99
# print(f"x-d: {x.__d}") # AttributeError: 'MyClass' object has no attribute '__d'

x.ShowNumber() # A: 2 	B: 2 	C: 2 	D: 2
y.ShowNumber() # A: 3 	B: 3 	C: 100 	D: 100
```

#### \_\_dict\_\_字典

在 Python 中，类和实例本质上都是对象，它们内部维护着一个名为 `__dict__` 的字典，存储了所有的属性。

```python
# 查看类的命名空间 
print(f"MyClass attributes: {MyClass.__dict__.keys()}") 
# 输出包含: static_a, _MyClass__private_static_b (注意名字变了), ShowNumber 等

>>> MyClass attributes: dict_keys(['__module__', 'static_a', '_MyClass__private_static_b', '__init__', 'ShowNumber', '__dict__', '__weakref__', '__doc__'])
```

```python
# 查看实例的命名空间 
print(f"x attributes: {x.__dict__.keys()}") 
# 输出包含: c, _MyClass__d

>>> x attributes: dict_keys(['c', '_MyClass__d'])
```

#### 对比总结

| 特性          | Python 代码示例               | Java 对应概念        | 核心差异                                                                                   |
| ----------- | ------------------------- | ---------------- | -------------------------------------------------------------------------------------- |
| **类变量**     | `static_a`                | `static` field   | Python 是动态语言，若通过 `self.static_a = ...` 赋值会变成实例属性（屏蔽类属性），必须通过 `MyClass.static_a` 修改才安全。 |
| **实例变量**    | `self.c`                  | Instance field   | 必须显式使用 `self` 定义。Java 可以声明在类体中，Python 必须在方法中（通常是 `__init__`）。                          |
| **私有类变量**   | `__private_static_b`      | `private static` | Python 通过改名为 `_MyClass__private_static_b` 实现“伪私有”。Java 是严格权限控制。                        |
| **私有实例变量**  | `self.__d`                | `private` field  | 同上。外部无法通过 `.__d` 访问，但在类内部完全可访问。                                                        |
| **构造器**     | `def __init__(self, ...)` | Constructor      | `__init__` 负责初始化，`__new__` 负责创建。Java 构造器与类同名。                                          |
| **self 参数** | `self`                    | `this`           | Python 必须显式声明为第一个参数。Java 是隐式传递。                                                        |
...
一个父类设计得非常抽象，以至于它都没有任何具体的实例，这样的类称为抽象类。

**在类头使用 abstract 关键字来声明一个类为抽象类。在抽象类中可以定义抽象方法（没有方法体，由子类实现），在方法头中使用 abstract 修饰符表示。**
...
public abstract class Parent {
    ...
    protected Parent(int x) {
        System.out.println("Parent constructor with int parameter");
    }

    public abstract void someMethod(); // 抽象方法，没有方法体
}
```

**注意要点：**

1. 不能使用 new 关键字创建抽象类的实例。
2. 抽象方法只有定义，没有实现。
3. 一个包含抽象方法的类必须声明为抽象类。
4. 抽象类的构造方法由 protected 修饰，因为只能被其子类调用。
5. 抽象方法不能包含在非抽象类中，子类必须实现所有的抽象方法，否则子类也必须声明为抽象类。
6. 子类可以覆盖父类方法并将其定义为 abstract （少见）。
7. 即使子类的父类是具体的，这个子类也可以是抽象的。
8. 不能使用 new 操作符从一个抽象类创建一个实例。但是抽象类可以作为一种数据类型。

```java
AbstractClass[] objects = new AbstractClass[3];
// 然后可以创建一个 AbstractClass 的实例，并将其引用赋值给数组
objects[0] = new Child();
```

#### 接口

接口是一种特殊的抽象类，它只包含抽象方法和常量，没有构造方法。接口定义了一个类应该实现的行为，而不关心实现细节。它的目的是**指明相关或不相关类的多个对象的共同行为。**

**在接口中，所有方法都是抽象的，所有字段都是常量（public static final）。**

使用 interface 关键字声明一个接口；然后，在类头使用 implements 关键字实现该接口。

```java
public interface InterfaceName {
    void someMethod(); // 抽象方法，没有方法体
}

// 带空体的接口，又称为标记接口（Marker Interface）
// 一个标记接口既不包括常量也不包括方法，只是用来标识某个类实现了某种行为或功能。
...
public class A implements InterfaceName {
    ...
    public void someMethod() {
        System.out.println("A implements InterfaceName");
    }
}

// A 和 B 类不相关，但都可以继承 InterfaceName 接口
public class B implements InterfaceName {
    @Override
    public void someMethod() {
        System.out.println("B implements InterfaceName");
    }
}
```
...
1. 接口不能包含实例字段（非静态字段），只能包含静态常量（public static final）。
2. 接口中的方法默认是 public abstract 的，不能使用其他访问修饰符。
...
public interface T {
    int x = 10; // 静态常量，默认是 public static final
    void p(); // 抽象方法，默认是 public abstract
}
```

3. 一个类可以实现多个接口，使用 implements 关键字。

```java
public class Child implements Interface1, Interface2, Interface3 { ... }
```

4. 实现接口的类必须实现所有的抽象方法，否则类必须声明为抽象类。
5. 接口可以继承多个接口，使用 extends 关键字。

```java
public interface Interface1 extends Interface2, Interface3 { ... }
```

6. 接口可以作为一种数据类型，用于声明引用变量。
